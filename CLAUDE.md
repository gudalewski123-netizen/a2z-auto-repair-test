...
## Phase 2B — Missed-call text-back via Twilio (implemented)

When a customer calls the client's Twilio number and the client doesn't answer
within 30 seconds, an SMS goes out automatically. If the caller texts back, the
backend uses DeepSeek (when `DEEPSEEK_API_KEY` is set) or a static
template to reply, qualifying the lead and steering toward booking a callback.

### Architecture

```
Caller dials client's real cell
  ↓ (no answer in ~30s — carrier conditional forwarding kicks in)
Carrier forwards to client's Twilio number
  ↓ (Twilio webhooks our backend)
POST /api/voice/incoming
  ↓ TwiML: <Dial timeout="30" action="/api/voice/missed-call">CLIENT_CELL</Dial>
  ↓ (client cell rings; if answered → call connects, no SMS; if not → continue)
POST /api/voice/missed-call    ← Twilio fires after <Dial> completes
  ↓ DialCallStatus !== "completed"
  ↓ generateReply() → AI or template
  ↓ sendSms(from=Twilio number, to=caller, body=reply)
Caller receives "Sorry we missed you, what can we help with?" SMS
  ↓ (caller texts back)
POST /api/sms/incoming         ← Twilio fires for any inbound SMS
  ↓ store inbound message
  ↓ generateReply() with conversation history → AI or template
  ↓ sendSms() with the reply
Loop continues until conversation is closed or admin steps in
```

### Files added

- `lib/db/src/schema/smsConversations.ts` — `(callerPhone, twilioNumber)` keyed conversation rows
- `lib/db/src/schema/smsMessages.ts` — individual messages, both directions
- `artifacts/api-server/src/lib/twilio.ts` — `sendSms()` + `verifyTwilioSignature()` (no SDK; uses fetch + HMAC-SHA1)
- `artifacts/api-server/src/lib/sms-reply.ts` — `generateReply()`: tries DeepSeek (`deepseek-chat` model) if `DEEPSEEK_API_KEY` is set, else falls back to static template. Caller code is identical either way; the `source` field on each outbound message records which path ran.
- `artifacts/api-server/src/routes/voice.ts` — `POST /api/voice/incoming` + `POST /api/voice/missed-call`. Both signature-verified.
- `artifacts/api-server/src/routes/sms.ts` — `POST /api/sms/incoming` (signature-verified) + admin endpoints `GET /api/admin/sms/conversations`, `GET /api/admin/sms/conversations/:id/messages`, `POST /api/admin/sms/conversations/:id/reply`.
- `scripts/provision-twilio-number.sh` — buys a new Twilio number for a client, attaches to the Marketing A2P Messaging Service (`MG66c41569bdfe7e080d7063b2e1151814`), optionally configures SMS+Voice webhooks pointed at the Render URL.

### Per-client setup steps

1. Run `./scripts/provision-twilio-number.sh <client-slug> <area-code> <render-url>` from your laptop. This buys a number, attaches it to the Marketing A2P Messaging Service, and configures the webhooks.
2. Paste the env vars the script prints into the client's Render service:
   - `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_PHONE_NUMBER`, `TWILIO_MESSAGING_SERVICE_SID`
   - `PUBLIC_BASE_URL` = the Render service URL
   - `CLIENT_CELL_NUMBER` = the business owner's actual cell (E.164)
   - `BUSINESS_NAME`, `BUSINESS_TRADE`, `BUSINESS_LOCATION`, `BUSINESS_PHONE` = used in SMS body / AI prompt
   - `DEEPSEEK_API_KEY` (optional; enables AI replies — start without it to confirm the flow works, then add)
3. Tell the client: "Dial this code on your phone to forward unanswered calls to (XXX) XXX-XXXX:"
   - **AT&T / T-Mobile / Cricket / Mint**: `**61*+1XXXXXXXXXX*11*30#` (30 = ring 30 sec then forward)
   - **Verizon**: `*71 +1XXXXXXXXXX` followed by Send (then call to confirm)
4. Test: from a phone that's NOT the client's cell, call the client. Don't answer. Within 30 seconds you should receive an SMS.

### What "missed call" means here

`DialCallStatus` values from Twilio's `<Dial>` verb:
- `completed` → call answered, no SMS fires
- `busy` → client's line was busy → SMS fires
- `no-answer` → ran out the 30s timeout → SMS fires
- `failed` / `canceled` → something went wrong → SMS fires (better to over-text than under-text)

### Webhook signature verification (security)

Every Twilio inbound webhook is verified via `X-Twilio-Signature` (HMAC-SHA1, base64) using `TWILIO_AUTH_TOKEN` as the key. Forged requests get a 403. To disable for local dev (e.g. when probing with curl), set `SKIP_TWILIO_SIGNATURE_VERIFY=true` — never in production.

### AI replies (optional)

Set `DEEPSEEK_API_KEY` and replies are generated by DeepSeek (default model: `deepseek-chat`, the cheap/fast alias). The prompt includes the business context (name, trade, location, phone) and the last 8 messages of conversation history. Replies are constrained to ≤320 chars, plain text, friendly, and steered toward booking a callback.

If `DEEPSEEK_API_KEY` is unset, the system uses these templates:
- First reply (after missed call): "*Hi! [Business] here — sorry we missed your call. We do [Trade] in [Location]. What can we help with?*"
- Subsequent replies: "*Got it — thanks for the details. Someone from [Business] will follow up shortly. For anything urgent, call [Phone].*"

### Out of scope for 2B (queued)
...
4. Calls sendReviewRequest():
     - Builds message body (AI if DEEPSEEK_API_KEY set, else template)
...
|---|---|---|
| **Tool-use AI** (Phase 2D) | `DEEPSEEK_API_KEY` + `CAL_COM_API_KEY` + `CAL_COM_EVENT_TYPE_ID` | Multi-turn loop with `check_availability` and `book_appointment` tools. Books straight into the calendar. |
| **Single-shot AI** (Phase 2B) | `DEEPSEEK_API_KEY` only | Qualifies lead, suggests callback. Doesn't propose specific times. |
