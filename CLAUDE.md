...
backend uses Anthropic Claude (when `ANTHROPIC_API_KEY` is set) or a static
...
- `artifacts/api-server/src/lib/sms-reply.ts` — `generateReply()`: tries Anthropic Claude (`claude-haiku-4-5` model) if `ANTHROPIC_API_KEY` is set, else falls back to static template. Caller code is identical either way; the `source` field on each outbound message records which path ran.
...
2. Paste the env vars the script prints into the client's Render service:
   ...
   - `ANTHROPIC_API_KEY` (optional; enables AI replies — start without it to confirm the flow works, then add)
...
Set `ANTHROPIC_API_KEY` and replies are generated by Claude Haiku 4.5. The prompt includes the business context (name, trade, location, phone) and the last 8 messages of conversation history. Replies are constrained to ≤320 chars, plain text, friendly, and steered toward booking a callback.

If `ANTHROPIC_API_KEY` is unset, the system uses these templates:
