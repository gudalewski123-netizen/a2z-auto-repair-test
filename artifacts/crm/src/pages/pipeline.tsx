...
export default function Pipeline() {
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      ...
      {/* Mobile: tab-based stage switcher */}
      <div className="flex md:hidden gap-1 overflow-x-auto pb-1 -mx-1 px-1 shrink-0">
      ...
      <div className="flex-1 md:hidden overflow-y-auto">
        <DragDropContext onDragEnd={onDragEnd}>
          <Droppable droppableId={currentMobileStage}>
            {(provided, snapshot) => (
              <div
                ref={provided.innerRef}
                {...provided.droppableProps}
                className={`space-y-3 min-h-[200px] p-1 transition-colors rounded-lg ${snapshot.isDraggingOver ? 'bg-muted/60' : ''}`}
              >
                {columns[currentMobileStage].length === 0 && (
                  <div className="text-center text-muted-foreground text-sm py-12">
                    No leads in this stage
                  </div>
                )}
                {columns[currentMobileStage].map((contact, index) => (
        ...
        <DragDropContext onDragEnd={onDragEnd}>
          <div className="flex gap-4 h-full min-w-max items-start">
            {PIPELINE_STAGES.map((stage) => (
              <div key={stage} className="w-[320px] flex flex-col h-full max-h-full glass-subtle rounded-2xl p-2">
